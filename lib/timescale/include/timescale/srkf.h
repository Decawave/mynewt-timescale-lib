...
typedef struct _srkf_instance_t{
	...
	void (*Aofx)(struct _srkf_instance_t * inst, double u[], double T);  /**< Callback function for Nonlinear state transition */
	void (*JofAx)(struct _srkf_instance_t * inst, double u[], double T); /**< Callback function for Jocobian of state transition */
	void (*Hofx)(struct _srkf_instance_t * inst, double T);  /**< Callback function for Nonlinear measurement equation */
	void (*JofHx)(struct _srkf_instance_t * inst, double T); /**< Callback function for Jocobian of measurement equation */
    void (*constraints)(struct _srkf_instance_t * inst, double T);    /**< Callback function constraints [optional] */
    void (*innovation)(struct _srkf_instance_t * inst, double T);    /**< Callback function innovation sequence [optional] */
    void *parent;              /**< Reference pointer for parent instance, use this to access derrived class variables */
    srkf_status_t status;        /**< Internal status and error flags*/
    srkf_ctrl_t ctrl;
}srkf_instance_t;

srkf_instance_t * srkf_init(srkf_instance_t * eke, double x0[], uint16_t n, uint16_t m);
void srkf_free(srkf_instance_t * eke);
srkf_status_t srkf_main(srkf_instance_t * eke, double z[], double u[], double T, uint16_t flag);
